...
public class PmCommandExecutor extends CommandExecutor {
    ...
    protected String executeValidCommand(String command, User user, List<User> usersList) {
        ...
        User userDestiny = availableUsers.get(new PromptMenu<Integer>()
                        .createNewMenu(
                                strArray,
                                Messages.getMessage("INFO_USERS_AVAILABLE"),
                                ChatCommandsMessagesTrafficManager.getUserPrompt(user))
                - 1);
        ...
        ChatCommandsMessagesTrafficManager.sendMessageToServer(String.format(
                Messages.getMessage("PLAYER_SENT_PM_TO_OTHER"),
                user.getUserName(),
                userDestiny.getUserName(),
                Colors.PURPLE_BOLD_BRIGHT,
                Colors.RESET));
        userDestiny
                .getClientDispatch()
                .notifyPlayer(String.format(
                        Messages.getMessage("PLAYER_MESSAGE_FOR_PM"), user.getUserName(), messageContent));
